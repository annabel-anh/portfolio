...
export default function NavLinks({ className, mobile = false, setOpen }) {
    const linkClass = mobile
        ? "-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 hover:bg-steel-blue/20"
    ...
    return (
        <div className={className}>
            {navigation.map((item) => (
                <a key={item.name} href={item.href} className={linkClass} onClick={handleLinkClick}>
                    {item.name}
                </a>
            ))}
        </div>
